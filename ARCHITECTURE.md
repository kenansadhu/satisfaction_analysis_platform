...
|------|-------------|----------------|
| 2026-05-17 | **NPS Display (Phase 2)**: Settings → "NPS Units" picker (`platform_settings.nps_unit_ids`) lets admins flag which organization units use the NPS scale. Marked units are excluded from cross-unit aggregations (`/unit-insights` list, `/api/executive/metrics`, `/api/executive/faculty-rollup`, `/api/ai/cache-global-dataset`) but still appear on the survey management page so their text columns can be analyzed via the standard flow. New `/api/executive/nps` endpoint returns per-unit + per-faculty NPS aggregates plus a qualitative summary (sentiment, top categories, sample quotes) for each NPS unit's text column. New `lib/nps.ts` provides shared bucketing (detractor 0–6 / passive 7–8 / promoter 9–10) and benchmark color helpers. New shared `NpsCard` and `NpsBucketBar` components. Executive Insights gains an "NPS" tab (per-unit cards, per-faculty table, qualitative breakdown with sample quotes) plus a small NPS tile in the dark hero strip. Faculty Insights list shows a per-faculty NPS strip on each card; the faculty detail page shows a full NPS card per (NPS unit × column). The AI Data Scientist cache (`ai_dataset_cache`) gets a new `nps_summary` section with per-unit and per-faculty NPS counts so the AI can chart NPS without ever blending it into Likert data. | `app/api/settings/nps-units/route.ts` [NEW], `lib/nps.ts` [NEW], `app/api/executive/nps/route.ts` [NEW], `components/nps/NpsCard.tsx` [NEW], `components/nps/NpsBucketBar.tsx` [NEW], `components/executive/NpsTab.tsx` [NEW], `app/executive/page.tsx`, `app/faculty-insights/page.tsx`, `app/faculty-insights/[facultyId]/page.tsx`, `app/settings/page.tsx`, `app/unit-insights/page.tsx`, `app/api/executive/metrics/route.ts`, `app/api/executive/faculty-rollup/route.ts`, `app/api/ai/cache-global-dataset/route.ts` |
| 2026-05-17 | **NPS score_rule (Phase 1 — isolation)**: Added `NPS_0_10` as a new `score_rule` value so 0–10 loyalty/recommendation questions can be flagged via the manage-survey UI (or auto-detected by `map-columns`). Import + manage recalculation now parse and validate 0–10 values. Every Likert satisfaction-index aggregation (`/api/executive/report`, `/api/executive/faculty-list`, `/api/executive/faculty-detail`, `/api/ai/cache-global-dataset`) explicitly filters `score_rule != 'NPS_0_10'` and adds a `max > 5` safety net so NPS values never pollute the 1–4 average. The AI prompt routes (`/api/ai/generate-report`, `/api/ai/chat-unit`) now compute NPS separately (detractor/passive/promoter bucketing) and present it as its own metric. **Action required:** run `supabase_nps_rpc_update.sql` in the Supabase SQL Editor to update `get_quant_summary_by_unit_campus` and invalidate stale cached aggregates. Display surfaces (executive overview + faculty insights) are Phase 2 and not yet implemented. | `types/index.ts`, `import/page.tsx`, `surveys/[id]/manage/page.tsx`, `api/ai/map-columns/route.ts`, `api/executive/report/route.ts`, `api/executive/faculty-list/route.ts`, `api/executive/faculty-detail/route.ts`, `api/ai/cache-global-dataset/route.ts`, `api/ai/generate-report/route.ts`, `api/ai/chat-unit/route.ts`, `supabase_nps_rpc_update.sql` [NEW] |
