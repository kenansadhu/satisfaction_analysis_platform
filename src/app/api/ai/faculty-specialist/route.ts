...
const chatKey = (facultyId: string, surveyId: string) => `faculty_ai_chat_${facultyId}_${surveyId}`;
...
export async function GET(req: NextRequest) {
    ...
    const [reportRes, chatRes] = await Promise.all([
        ...
        supabase.from("survey_misc_cache").select("data").eq("survey_id", sid).eq("cache_key", chatKey(facultyId, surveyId)).maybeSingle(),
    ]);
...
export async function POST(req: NextRequest) {
    const { facultyId, surveyId, messages } = await req.json();
    if (!facultyId || !surveyId) return NextResponse.json({ error: "required" }, { status: 400 });

    const { error } = await supabase.from("survey_misc_cache").upsert(
        { survey_id: parseInt(surveyId), cache_key: chatKey(facultyId, surveyId), data: { messages }, updated_at: new Date().toISOString() },
