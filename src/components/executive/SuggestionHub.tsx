...
type StructuredSummary = {
    ...
    cross_category_patterns: { unified_theme: string; detail: string; sources: { unit: string; category: string; count: number }[] }[];
        ...
        try {
            const res = await fetch("/api/executive/suggestions-summary", {
                ...
                body: JSON.stringify({
                    ...
                    themes: themes.slice(0, 40).map(t => ({
                        unit: t.unit.name,
                        category: t.category,
                        count: t.count,
                        negative: t.negative,
                        ...
                        quotes: [...t.suggestions]
                            .sort((a, b) => {
                                const rank = (s: typeof a) => s.sentiment === "Negative" ? 0 : s.sentiment === "Neutral" ? 1 : 2;
                                if (rank(a) !== rank(b)) return rank(a) - rank(b);
                                return b.text.length - a.text.length;
                            })
                            .slice(0, 15)
                            .map(s => ({ text: s.text, sentiment: s.sentiment })),
                    })),
                    category_matrix: themes.reduce((acc, t) => {
                        const u = t.unit.name;
                        if (!acc[u]) acc[u] = [];
                        acc[u].push({ category: t.category, count: t.count, negative: t.negative });
                        return acc;
                    }, {} as Record<string, { category: string; count: number; negative: number }[]>),
                }),
            });
        ...
        return (
            <div key={theme.key}>
                ...
                >
                    ...
                    <Badge variant="outline" className={`text-xs font-semibold px-2 border w-20 shrink-0 flex items-center justify-center gap-1 ${style.badge}`}>
                        {priority === "High" ? <AlertTriangle className="w-3 h-3" /> : priority === "Medium" ? <Minus className="w-3 h-3" /> : <TrendingUp className="w-3 h-3" />}
                        {priority}
                    </Badge>

                    {/* Voice count */}
                    <span className="w-10 text-center text-base font-bold tabular-nums text-slate-700 dark:text-slate-300 shrink-0">{theme.count}</span>
                    ...
                    <div className="flex-1 min-w-0">
                        {summaryLoading ? (
                            ...
                            <span className="text-sm text-slate-700 dark:text-slate-300 truncate block">{aiLine}</span>
                        ) : (
                            <span className="text-sm text-slate-500 dark:text-slate-400 italic truncate block">"{theme.topQuote}"</span>
                        )}
                    </div>

                    {/* Sentiment bar */}
                    <div className="hidden lg:flex items-center gap-2 shrink-0 w-28">
                        <div className="flex h-2 flex-1 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                        ...
                        <span className="text-xs text-slate-400 w-8 text-right">{negPct}%</span>
                    </div>
                ...
                {/* Expanded quotes */}
                {isOpen && (
                    <div className="bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 px-5 py-5 space-y-3 animate-in slide-in-from-top-1 fade-in duration-150">
                        ...
                        {theme.suggestions.map(s => (
                            <div key={s.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-5 py-4">
                                <p className="text-base text-slate-700 dark:text-slate-300 leading-relaxed">"{s.text}"</p>
                                <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                                    ...
                                    {s.context.faculty && <span className="text-xs text-slate-400">{s.context.faculty}</span>}
                                    {s.context.program && <span className="text-xs text-slate-400">· {s.context.program}</span>}
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg w-fit">
                {([
                    ...
                    >
                        ...
                        {label}
                        {key === "patterns" && (aiSummary?.cross_category_patterns?.length ?? 0) > 0 && (
                            <span className="ml-1 px-1.5 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-700 rounded-full">{aiSummary!.cross_category_patterns.length}</span>
                        )}
            ...
            {viewMode === "summary" && (() => {
                ...
                if (!aiSummary && !aiLoading) return (
                    <div className="bg-gradient-to-br from-violet-50 to-indigo-50 dark:from-violet-950/20 dark:to-indigo-950/20 border border-violet-200 dark:border-violet-800 rounded-xl p-14 flex flex-col items-center text-center gap-5">
                        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-violet-200 dark:border-violet-700 shadow-sm">
                            <Sparkles className="w-10 h-10 text-violet-500" />
                        </div>
                        <div>
                            <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-200 mb-2">No summary yet</h3>
                            <p className="text-base text-slate-500 max-w-sm">Generate an AI-powered executive briefing covering top issues, bright spots, and recommended actions.</p>
                        </div>
                        <Button size="lg" className="gap-2 bg-violet-600 hover:bg-violet-700 text-white text-base" onClick={handleGenerateSummary}>
                            <Sparkles className="w-5 h-5" /> Generate Summary
                        </Button>
                        {aiError && (
                            <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-3 w-full max-w-md">
                ...
                if (aiLoading) return (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        ...
                        <p className="text-base text-slate-500 font-medium animate-pulse">Generating executive briefing…</p>
                    </div>
                );
                ...
                return (
                    <div className="space-y-5">
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-violet-100 dark:border-violet-900 rounded-xl px-6 py-5 flex items-start justify-between gap-4">
                            <p className="text-lg font-semibold text-slate-800 dark:text-slate-200 leading-snug">{s.headline}</p>
                            <div className="flex items-center gap-2 shrink-0">
                                <span className={`text-sm font-bold px-3 py-1 rounded-full border ${moodStyle[s.overall_mood] || moodStyle.Mixed}`}>{s.overall_mood}</span>
                                <Button size="sm" variant="outline" className="gap-1.5 text-sm border-violet-300 text-violet-700 hover:bg-violet-50" onClick={handleGenerateSummary} disabled={aiLoading}>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div className="space-y-3">
                                <div className="flex items-center gap-2 px-1">
                                    ...
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Top Issues</span>
                                </div>
                                {(s.top_issues || []).map((issue, i) => (
                                    <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
                                        <div className="flex items-start justify-between gap-2 mb-2">
                                            <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 leading-tight">{issue.title}</span>
                                            <span className={`text-[11px] font-bold px-2 py-0.5 rounded border shrink-0 ${urgencyStyle[issue.urgency] || urgencyStyle.Medium}`}>{issue.urgency}</span>
                                        </div>
                                        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-3">{issue.detail}</p>
                                        {issue.units_affected?.length > 0 && (
                                            <div className="flex flex-wrap gap-1.5 mb-3">
                                                {issue.units_affected.map((u, j) => (
                                                    <span key={j} className="text-xs font-medium px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-full">{u}</span>
                                                ))}
                                            </div>
                                        )}
                                        <p className="text-xs text-slate-500 italic border-l-2 border-slate-200 pl-3 leading-relaxed">"{issue.evidence}"</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-2 px-1">
                                    ...
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Bright Spots</span>
                                </div>
                                {(s.bright_spots || []).map((spot, i) => (
                                    <div key={i} className="bg-white dark:bg-slate-900 border border-emerald-100 dark:border-emerald-900 rounded-xl p-4">
                                        <span className="text-sm font-semibold text-emerald-700 dark:text-emerald-400 block mb-1.5">{spot.title}</span>
                                        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-3">{spot.detail}</p>
                                        <p className="text-xs text-slate-500 italic border-l-2 border-emerald-200 pl-3 leading-relaxed">"{spot.evidence}"</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Recommended Actions</span>
                            </div>
                            <div className="space-y-3">
                                {(s.recommended_actions || []).map((action, i) => {
                                    ...
                                    return (
                                        <div key={i} className="flex items-start gap-4">
                                            <span className={`flex items-center gap-1.5 text-xs font-bold px-2.5 py-1.5 rounded border shrink-0 ${timelineStyle[action.timeline] || timelineStyle["Long-term"]}`}>
                                                <Icon className="w-3 h-3" />{action.timeline}
                                            </span>
                                            <div>
                                                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-0.5">{action.action}</p>
                                                <p className="text-sm text-slate-500 leading-relaxed">{action.rationale}</p>
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 italic text-center px-4 pb-1">{s.closing_statement}</p>
                    </div>
                );
            })()}
            ...
            {viewMode === "themes" ? (
                <>
                    ...
                    {summaryError && (
                        <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-3">
                            <AlertTriangle className="w-4 h-4 shrink-0" /> {summaryError}
                        </div>
                    )}

                    {/* Column header */}
                    {filtered.length > 0 && (
                        <div className="flex items-center gap-3 px-4 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wide border-b border-slate-200 dark:border-slate-800">
                            <span className="w-20 shrink-0">Priority</span>
                            <span className="w-10 text-center shrink-0">N</span>
                            <span className="w-48 shrink-0">Unit / Category</span>
                                ...
                                <div key={groupName}>
                                    <div className="flex items-center gap-2 mb-1">
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-full">
                                            {groupBy === "unit" ? <Building2 className="w-3.5 h-3.5 text-slate-500" /> : <Tag className="w-3.5 h-3.5 text-slate-500" />}
                                            <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">{groupName}</span>
                                            <span className="text-xs text-slate-400">({groupThemes.length})</span>
                                        </div>
                ...
                /* Hidden Connections — AI-detected cross-category semantic patterns */
                <div className="space-y-4">
                    {!aiSummary?.cross_category_patterns?.length ? (
                        <div className="text-center py-16 text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
                            ...
                            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No hidden connections yet</p>
                            <p className="text-xs mt-1 text-slate-400">
                                Generate an AI Summary first — the AI will detect categories across different units that describe the same underlying issue.
                            </p>
                            <button
                                className="mt-4 text-xs font-semibold text-violet-600 dark:text-violet-400 underline underline-offset-2 hover:text-violet-800"
                        ...
                        <>
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                The AI found{" "}
                                <span className="font-semibold text-slate-700 dark:text-slate-200">{aiSummary.cross_category_patterns.length}</span>{" "}
                                hidden connection{aiSummary.cross_category_patterns.length !== 1 ? "s" : ""} — different category names across units that describe the same underlying concern.
                            </p>
                            <div className="space-y-3">
                                {aiSummary.cross_category_patterns.map((p, i) => (
                                    <div key={i} className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-900/50 rounded-xl p-5">
                                        <div className="flex items-start gap-3 mb-3">
                                            <div className="flex items-center justify-center w-7 h-7 rounded-full bg-amber-100 dark:bg-amber-950/50 shrink-0 mt-0.5">
                                                <GitMerge className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
                                            </div>
                                            <div className="min-w-0">
                                                <p className="text-sm font-bold text-amber-800 dark:text-amber-300 leading-snug">{p.unified_theme}</p>
                                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mt-1">{p.detail}</p>
                                            </div>
                                        </div>
                                        <div className="pl-10">
                                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Source categories</p>
                                            <div className="flex flex-wrap gap-2">
                                                {(p.sources || []).map((src, j) => (
                                                    <div key={j} className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
                                                        <span className="font-semibold text-slate-700 dark:text-slate-300">{src.unit}</span>
                                                        <span className="text-amber-400">·</span>
                                                        <span className="text-amber-700 dark:text-amber-400 italic">"{src.category}"</span>
                                                        <span className="text-slate-400 font-medium">({src.count})</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
